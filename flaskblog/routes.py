...
def home2():
    ...
    if current_user.accept_flag==True:
        if current_user.usertype=="Consumer":
            ...
            flash('Your recent Offer for Providing Food has been Accepted. Check Notifications & Generate Key for Number Lock !!', 'success')
...
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        post = Post(food=form.food.data, author=current_user)
        db.session.add(post)
        db.session.commit()
...
@app.route("/find/<int:post_idyo>/<string:post_food>")
def find(post_idyo, post_food):
    print("Executing find")
    page=request.args.get('page',1,type=int)
    post= Post.query.get_or_404(post_idyo)
    posts = Post.query.order_by(Post.date_posted.desc()).paginate(per_page=100)
    return render_template('find.html', posts=posts, post_idyo=post_idyo, post_food=post_food)
